...
const LightPillar = ({
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        vec2 noiseCoord = vUv * uResolution + uTime;
        float noiseValue = noise(noiseCoord * uNoiseIntensity) * 0.1;
        color += noiseValue;
  ...
  if (!webGLSupported) {
    return (
      <div className={`${styles.fallback} ${className}`} style={{ mixBlendMode }}>
        WebGL not supported
      </div>
    );
  }

  return <div ref={containerRef} className={`${styles.container} ${className}`} style={{ mixBlendMode }} />;
};
